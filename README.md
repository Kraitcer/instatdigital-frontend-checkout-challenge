...
Фронтенд запускается отдельно после запуска API:

```sh
npm run dev:web
```

Приложение будет доступно на [http://127.0.0.1:5173](http://127.0.0.1:5173). Сборка фронтенда проверяется командой `npm run build:web`.

## Устройство фронтенда

Страницы каталога, оформления и результата находятся на маршрутах `/`, `/checkout` и `/success`. Общие поля, состояния ошибок и загрузки, карточки, корзина и платёжная форма вынесены в компоненты в `apps/web/src/components`.

Все endpoint-описания собраны в `apps/web/src/api/checkoutApi.ts`. RTK Query вызывает единый `baseQuery`, а тот передаёт запрос общему транспорту `apps/web/src/api/client.ts`. Только транспорт задаёт base URL и заголовки, сериализует тело, проверяет HTTP-статус, разбирает envelope и приводит сетевые, HTTP- и parse-ошибки к `ApiFailure`. Компоненты получают типизированные данные или уже нормализованную ошибку. RTK Query дедуплицирует чтения, инвалидирует корзину после изменений и отменяет polling при размонтировании страницы.

Redux хранит клиентский workflow: сессию, идентификаторы текущего заказа и платежа, идемпотентные ключи и введённую форму. `redux-persist` сохраняет этот reducer в `localStorage`; серверные сущности остаются в кэше RTK Query и после перезагрузки заново подтверждаются API. Валидация формы описана одной Zod-схемой и подключена через React Hook Form.

### Обработка каталога

При каждом новом ответе корзины `indexCartItems` один раз проходит её позиции за $O(C)$ и создаёт один `Map` из $C$ записей. Рендер каталога затем один раз проходит $P$ товаров и получает количество за ожидаемое $O(1)$ на товар. Итого $O(C + P)$ времени и $O(C)$ дополнительной памяти без `find` внутри `map`, промежуточных `filter`-массивов и копирования растущего результата. Индекс пересоздаётся только при изменении ссылки на корзину благодаря `useMemo`.
